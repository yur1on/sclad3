...
def compress_image(image):
    ...
    Обработка изображения:
      - Изменение размера до максимума 1280 пикселей по большей стороне.
      - Конвертация в формат JPEG с качеством 75.
    ...
    output_io = BytesIO()
    img.save(output_io, format="JPEG", quality=75)  # Было 85
...
def add_watermark_to_image(image_file, watermark_text="MobiRazbor.by", opacity=120):
    ...
    output_io = BytesIO()
    watermarked_image.save(output_io, format="JPEG", quality=75)  # Было 90
